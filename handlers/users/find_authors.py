...
async def author_command(message: types.Message | FSMContextProxy):
    ...
    url = f'https://flibusta.is//booksearch?ask={author}&cha=on'

    current_author_hash = create_current_name(
        message.chat.type, author.title())
...
async def current_languages(msg: types.CallbackQuery | types.Message, callback_data: dict):
    ...
    url = f'https://flibusta.is{link}&lang={language}&order=p&hg1=1&hg=1&sa1=1&hr1=1'

    current_author_link = create_current_name(
        message.chat.type, link + language, flag=True)
...
async def show_chosen_page(call: types.CallbackQuery, callback_data: dict):
    ...
    markup = get_small_keyboard(count_pages=len(
        authors_lst), key=current_author, page=current_page, method='author')
    try:
